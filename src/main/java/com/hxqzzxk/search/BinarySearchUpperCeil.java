 ...
 */
public class BinarySearchUpperCeil<E extends Comparable<E>> implements BinarySearch<E> {
    /**
     * 大于 target 时，获取最小值的索引
     * 等于 target 时，获取所有 target 中最大的索引
     *
     ...
     * @return 大于 target 时，返回最小值的索引，等于 target 时，返回所有 target 中最大的索引
     */
    @Override
    public int search(E[] elements, E target) {
        int upperIndex = upper(elements, target);
        if (upperIndex - 1 >= 0 && elements[upperIndex - 1].compareTo(target) == 0) {
            return upperIndex - 1;
        }
        return upperIndex;
    }
     ...
     */
    private int upper(E[] elements, E target) {
        int left = 0;
